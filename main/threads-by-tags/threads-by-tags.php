...
<html>
    <head>
        ...
        <!-- タグ別スレッド一覧用SS  -->
        <link rel="stylesheet" type="text/css" href="./threads-by-tags-style.css">
    </head>
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="main">
                <div class="content">
                    <!-- タグ別スレッド一覧  -->
                    <div id="tags_search_result">
                        <h1 class="result-sentence" id=result_sentence></h1>
                        <dl id="tags_search_result_list"></dl>
                    </div>
                </div>
            </div>
            ...
            <script type="text/javascript">
                ...
                $.ajax({
                    type: "POST",
                    url: "./SearchThreadsByTagsBK.php",
                    data: {
                        'tag_id': tag_id,
                    }
                })
                .done(function(data){
                    $("#result_sentence").text("タグ:"+data[0].tag+" のスレッド一覧");
                    if (data[1].id != null)
                    {
                        for (let i = 1; i < data.length; i++)
                        {
                            ...
                            $("#tags_search_result_list").append(h);
                        }
                    } else {
                        $("#tags_search_result_list").append("<h3 class='not-thread'>スレッドは存在しません。</h3>");
                    }
                })
            </script>
